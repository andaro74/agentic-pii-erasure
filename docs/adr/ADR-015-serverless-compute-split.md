...
- **Positive — the planes can be upgraded independently.** A new discovery artifact does not redeploy the saga, and vice versa. *(Written as "container image"; [ADR-025](ADR-025-runtime-ships-a-code-zip.md) changed the artifact to an S3 code zip. The independence claim is unaffected — it rests on the two planes being separate stacks, not on what either ships.)*
...
- **Cost 2 — two deployment artifacts.** ~~A container image in ECR for the Runtime~~ **an S3 code zip for the Runtime** ([ADR-025](ADR-025-runtime-ships-a-code-zip.md) — the container was the only artifact `CreateAgentRuntime` accepted when this was written), zipped Lambdas for the control plane. `infra/` owns both; `make deploy-dev` does both. The cost is smaller than stated: both artifacts are now zips built by the same `make package` step, and there is no ECR repository to provision or bill.

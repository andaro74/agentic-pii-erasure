...
- **Positive — residual honesty gets teeth.** `notify-suppression` genuinely cannot delete the suppression entry; `analytics-lake` genuinely retains rows until snapshot expiry. `PARTIAL` + `residual` stops being a modelled behaviour and becomes an observed one.

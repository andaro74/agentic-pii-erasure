...
### 2026-07-28 · V11-4 — the walkthrough erased a subject that never existed

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V11-4a** | **High** | **`walkthrough` invented its subject.** It generated `sub_<uuid>` and started a saga for it. Discovery ran correctly, found the handle nowhere, and returned zero participants; `plan` then called `Manifest.model_validate` on a body with an empty `participants`, which `Manifest` rejects by design (`min_length=1`). The saga died inside a graph node 37 seconds in. | `seeded_subject()` reads `evals/fixtures/ground-truth.json` — the map `make seed` emits in the same pass as the writes (ADR-020) — and picks the subject touching the most systems. A missing map says "run `make seed` first" instead of inventing a handle. |
| **V11-4b** | **High** | **An empty discovery crashed instead of terminating.** Independent of the walkthrough: a controller who holds no data on a subject still owes them an answer under Art. 12(3), and "nothing to erase" is a legitimate outcome. It was an unhandled pydantic `ValidationError` mid-graph. | `plan` detects the empty sweep, appends `DISCOVERY_FOUND_NOTHING`, and returns `STATUS_NO_DATA`; `PATH_MAPS["plan"]` routes it to `END`. Kept **distinct from `completed`** on purpose — a certificate must never imply deletions that never happened. |
| **V11-4c** | **High** | **The certificate check could never pass.** `_certificate` required lowercase `"approval_granted"` and `"hard_delete_applied"`; the nodes emit `APPROVAL_GRANTED` and `HARD_DELETE_APPLIED`. A flawless run would have failed at step 8 with "no ledger entry evidencing the deletion" — the most alarming possible way to be wrong. | Exact uppercase names in a frozenset, and `tests/unit/test_walkthrough_certificate.py` greps `saga/nodes/` for the `event_type="..."` strings actually emitted and fails if a required one is absent. A rename now breaks a unit test rather than a deployed gate. |
| **V11-4d** | **Low** | **The poll printed nothing for up to fifteen minutes**, so correct waiting and a hang looked identical. This is how the defect was reported: *"why does the terminal get stuck?"* | `wait_for` takes a `notify` callback and reports on every state change and at least once a minute. |

**Three of these four are the same failure of nerve: the walkthrough was never run.** It could not be — `make walkthrough` is human-only and costs money — but that is an argument for making its *checkable parts* hermetic, not for shipping them unexercised. The certificate assertion and the subject selection are both pure functions over data, and both now have unit tests that would have caught the defect before the deploy. The rule this earns: **when a deployed gate contains logic, that logic gets hermetic tests even though the gate itself cannot run** — otherwise the first execution of the code is also its first test, on the human's time.
...
> **Resolved by [ADR-027](adr/ADR-027-holds-block-a-scope-not-a-subject.md)** during M9: a hold blocks its scope, not the subject. The rule moved to `contract/holds.py` so participants and the saga read it from one place. Writing the chaos case that exercises it then surfaced **V12-2** below — the decision was made in `hold_check`/`hold_recheck` and not extended to the two nodes downstream of them.

### 2026-07-29 · V12-1 — a dev override that could only lengthen the window it was added to shorten

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V12-1** | **High** | **`GRACE_SECONDS_OVERRIDE` replaced the manifest's window instead of capping it.** [V11-1](#2026-07-28--v11-1--the-only-path-that-needed-the-variable-never-exercised-it) added it so a dev saga would not sit at the grace gate for the manifest's 30 days. Read as a replacement it also overrode a deliberate **zero**: every fixture manifest sets `graceWindowDays: 0`, so on a dev stack `make integration` — M5's ticked deployed gate — paused at a gate its assertions say is skipped, and `_run_sweeps_to_completion` then delivered `sweep_t7` to an interrupt expecting `grace_elapsed`. Three of its four tests could no longer pass. | The override is now a **ceiling**: `min(manifest_seconds, override)`. It may only shorten a window a manifest asked for, never lengthen it — which is what `.env.example` has promised since M0 ("compress via this parameter only") and what `grace_window.py`'s own docstring already claimed ("a zero-length window — fixture manifests, dev overrides — skips the pause entirely"). Two hermetic tests: the deployed dev configuration reaches the sweep gate, and a 30-day window still compresses to a real 120-second timer. |

**An M8 fix silently invalidated an M5 gate, and nothing existed to notice.** The hermetic rig defaults `grace_seconds_override=None`, so `make check` exercised a configuration no deployed stack runs; the deployed suite that would have failed costs money and had already been ticked. This is V11-1's own diagnosis recurring one milestone later — *the one path that needed this was the one path that never exercised it* — and the fix is the same shape: a hermetic test that drives the graph with the value the stack actually sets.

The general form is worth stating, because it is not about grace windows: **ticking a deployed gate records that it passed once, not that it still passes.** Nothing in this repo re-runs M5's gate when M8 changes the stack. `make chaos` now overlaps it deliberately (four of its seven cases *are* `test_saga.py`'s), which narrows the window without closing it.

### 2026-07-29 · V12-2 — ADR-027 was implemented in the node that decides and not in the two that follow

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V12-2** | **High** | **A residual retained under a scoped hold was graded as a failed erasure.** ADR-027 lets phase 3 proceed past a hold that covers only part of the plan; `billing-ledger` then keeps `public.invoices` under Art. 17(3)(e), returns `PARTIAL` with a residual naming it, and reports those rows — correctly — at `verify`. But `verify_all_participants` excused remaining artifacts **only** for participants flagged `expects_residual` in the registry, and billing is not one: its residue is not structural, it is one hold on one saga. So the T+0 verify DLQ'd a *legally required* retention as unexpected residue and halted the saga at `stuck`, making ADR-027's "proceeds for the rest" unreachable end to end. At T+7 the same grading would have raised a **`RESURRECTION_INCIDENT`** — a security escalation — for data that never left. | Remaining artifacts are now also excused when **the participant itself disclosed them** in its own phase-3 receipt, matched by locator *and count*. Invariant 7 obliges participants to disclose residue honestly; the platform's half of that bargain is to treat what was disclosed as disclosed. The count comparison keeps the exemption from becoming a blind spot: five rows under a locator whose hold retained three is data that came back, and is still a resurrection. |

**Found by writing the chaos case, not by running it** — the deployed gate this belongs to has not run yet. The two cases M9's roadmap line lists side by side turn out to collide: the hold-during-grace-window arc ends in the sweep that the resurrection arc is about, and under the old grading the first would have reported the second's incident.
...
### 2026-07-29 · V12-6 — the release gate could never have passed, and the check added an hour earlier is what said so

**Found by running `bash scripts/upgrade_canary.sh 1.2.10 1.2.0`** — the first real attempt at M9's second gate.

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V12-6a** | **High** | **The canary bumped two of the three places a pin lives.** `SAGA_PINS` in the Makefile is what `make package` installs into the saga Lambda asset — a separate string on purpose, so a reader can see what ships, with `test_saga_pins.py` comparing it against pyproject verbatim. The canary rewrote `pyproject.toml` and `requirements.lock` and left it. Not a quiet mis-ship: `make lock` writes the new version into the lockfile, `make package` passes that as a constraint, and pip becomes unresolvable at step 3. **The release gate ADR-016 calls its only real control was structurally incapable of passing.** | The script bumps `SAGA_PINS` too, asserts all three moved, and `restore()` puts the Makefile back. The contract test now **derives** the set of pin-bearing files from the tree and asserts the script rewrites each — so a fourth location fails in `make check` rather than at the next canary. |
| **V12-6b** | **Medium** | **`restore()` restored the files and left the venv upgraded.** After the failure the tree said 1.2.9 and the environment held 1.2.10, so every subsequent `make check` failed on `test_the_installed_versions_match_the_pins` — correctly, and for a reason with nothing to do with whatever the operator did next. | The venv is still deliberately *not* rolled back — a failure is worth inspecting on the versions that caused it — but `restore()` now says so and names `make install`. Asserted by a test, because a deliberate state that reads as a mess is indistinguishable from a mess. |

**The interesting part is what caught it.** `make check`-before-deploy went into this script an hour earlier, on general grounds: a version that breaks an API we call should cost named unit failures rather than a five-minute deploy and a confusing resume. It then caught a defect in the script that added it, two steps before the deploy, with the precise message `SAGA_PINS and pyproject.toml disagree`. Without it the run would have failed at `make package` on an opaque pip resolution error, after `make lock` and `make install` had already churned the venv.

**Three pins, and the count is the finding.** `test_saga_pins.py` has guarded the pyproject↔Makefile pair since M5 and its docstring is explicit that *"two places name the langgraph pins"*. The canary was written against the invariant's prose — *"pins `langgraph` and `langgraph-checkpoint-aws` to exact versions"* — which names the two **packages** and not the places they are written. Two axes, silently conflated: the script covered both packages in one file and called it both pins. That is the same shape as [V12-2](#2026-07-29--v12-2--adr-027-was-implemented-in-the-node-that-decides-and-not-in-the-two-that-follow) and [V12-3](#2026-07-29--v12-3--the-fake-had-no-foreign-keys-so-it-certified-a-statement-the-database-refuses), and it is now the sixth in this pass. The durable fix is not the extra `sed` — it is that the test derives the file list instead of restating it.

### 2026-07-29 · V12-7 — the least pinned thing in a repo whose whole point is pinning

**Found by the second real canary run**, one step further than V12-6 got: `make lock` failed at `Makefile:56`.

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V12-7a** | **High** | **`make lock` — the mechanism invariant 9 names — installed its own tooling unpinned, and was broken.** `pip install -q pip-tools` against whatever pip the venv had; pip-tools imports `pip._internal.utils.compat.stdlib_pkgs`, a **private** symbol pip 26 removed. With pip 26.2 installed, both `python -m piptools compile` and the `pip-compile` console script raise `ImportError` — the latter because config validation imports `sync` too. pip-tools 7.6.0 is the newest release, so there was nothing to upgrade to. `make lock` is called by exactly one thing, this canary, so a repo built around exact pins could not regenerate its own lockfile. | Both tools pinned (`"pip<26" "pip-tools==7.6.0"`) and installed into a **throwaway venv**: locking must not depend on which pip a developer has, nor mutate the project venv's pip as a side effect of regenerating a lockfile. Verified by running the real target — exit 0, tool venv torn down, and `requirements.lock` byte-identical, which is the correct no-op when no pin moved. |
| **V12-7b** | **Low** | **`restore()` warned unconditionally that the venv held the target versions.** `make lock` runs *before* `make install`, so this failure never touched the environment — and the warning sent an operator to reinstall something nothing had changed. | Guarded on a flag set when `make install` actually runs; otherwise it states the venv was untouched. A true warning that fires when it does not apply is how a true warning stops being read. |

**"Pinned" was a property of the artefacts, never of the toolchain that produces them.** Invariant 9 is about `langgraph` and `langgraph-checkpoint-aws`; nothing asked what version of `pip-tools` computed `requirements.lock`, or what `pip` it imported private symbols from. The failure is environment-dependent in the worst way — it works on a machine with an older pip and breaks on a newer one, so it is invisible until someone else runs the one target that uses it.
...
### 2026-07-30 · V13 — M10 opened on fourteen controls that could not have worked

Most of these were found by *building the next layer down* rather than by reading, which is the pass's pattern: an alarm forced me to ask what publishes the metric, and the answer was "nothing, and the scrubber would have corrupted the name anyway". Two came from wiring the final pair of emitters, where the question "what number will this actually produce?" turned out to have an answer nobody had computed. The eighth came from the closing validation pass, by asking the only question the other seven had left: not *what* is published, but *under which dimensions*.

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| **V13-1** | **High** | **`scripts/upgrade_canary.sh` hardcoded `.venv/Scripts/python.exe`** on both pytest invocations. The `upgrade-canary` job in `ci.yml` runs on `ubuntu-latest`, where that path does not exist, so the job would have died on its first line. The Makefile has auto-detected the venv layout since M0; the script never did, and it had never run in CI. | Resolves `$VENV_PY` for both layouts, fails loudly if neither exists, and a test rejects any line that invokes a platform-specific interpreter directly. |
| **V13-2** | **High** | **The CI canary job could not canary an upgrade.** It ran `make upgrade-canary` with no arguments — the rehearsal form. The job fires *because* the pins changed, so its checkout already holds the new versions: it paused and resumed on the same version and reported success for a transition it never tested. The direction was backwards. | The job rolls all three pin files back to the base commit **before** `make install`, then hands the PR's versions to the canary as targets. `tests/unit/test_workflow_contract.py` asserts the ordering, the argument passing, and that every `run:` step is valid shell. |
| **V13-3** | **High** | **The PII scrubber rewrote EMF metric names.** Embedded metric format puts the metric name in a member called `Name`, and `name` is a denied key — correctly, for a person's name. So `{"Name": "resurrection.detected"}` became `{"Name": "[REDACTED]"}`, CloudWatch would have extracted a metric literally called `[REDACTED]`, and **every alarm in M10 would have been unable to fire while rendering green.** | The `_aws` metadata node is exempt, because it holds only *references* — namespace, dimension key names, metric names, units. Every dimension and metric **value** lives on the root node and is still scrubbed. `emit()` validates the node where it is built, so the exemption is not a hiding place. |
| **V13-4** | **Medium** | **A hard constraint had an unrecorded exception.** CLAUDE.md, ARCHITECTURE §14.2 and `infra/README.md` all said *"nothing in the stack bills continuously for existing rather than for working"*. One thing does: a Secrets Manager secret, $0.40/month, created for the Aurora credentials. The claim was not *wrong* about the magnitude — an idle stack does cost cents — but ADR-021's procedure ("a provisioned floor needs an ADR arguing why the floor is worth it") had never been applied to it. | All three docs now name the exception and why it is forced rather than chosen: the RDS Data API authenticates with a secret ARN, and the Data API is what keeps every Lambda out of a VPC. `tests/unit/test_cost_floors.py` makes ADR-021's rule mechanical. |
| **V13-5** | **Medium** | **§12's "Scheduler never fires" row named a detection that cannot happen.** It was covered by the `saga.duration` alarm — but every metric this platform publishes is emitted *by a saga node*, and a saga whose timer never fires runs no node. No data point, no alarm, no detection. The row described the intention (*somebody should notice a silent saga*) in the vocabulary of a mechanism. | The row now names the control that exists: `make threads` reads every thread from the checkpointer and shows what each is waiting for, run by an operator. The automated version — a scheduled scan for non-terminal threads older than their gate allows — is named in §12 as a real gap rather than implied by an alarm that would sit green forever. Not built: it is a new component and a new schedule, and M10's Build line does not contain one. |
| **V13-6** | **Medium** | **`saga.duration`'s specified alarm would have fired on every healthy saga.** §10.1 asks for `> statutory deadline − 7d` (23 days). The default grace window is 30 days, so a completely successful erasure reaches its T+0 verification *after* the one-month deadline — the alarm would have been red permanently from the first saga. Worse, the naive measurement point (the graph's END) is reached after the **T+30** re-verification sweep, adding another month to a number that already breached. Neither had been computed; the threshold was copied from the compliance requirement, which is not the same thing as the system's behaviour. | Measured intake → **T+0 verification**, the moment the subject's request is actually answered, with a test that fails if the measurement point moves to the sweeps. Emitted and dashboarded with **`threshold=None`**, so no alarm is created; `NOT_ALARMED` records why, and a test requires every unalarmed metric to. §16 Q4 — the grace-window/deadline conflict — is updated to record that it is now an arithmetic obstruction to a control rather than only a caveat. |
| **V13-7** | **High** | **`tests/unit/test_cost_floors.py` could not have passed in CI — and it is two commits old.** `make check` ran `lint test policy-test synth`, so the unit suite ran **before** `cdk synth`. The check reads `infra/cdk.out`, which is gitignored and absent on CI's clean checkout: the fixture would have failed the hermetic job on its first run. It passed locally for the only reason that matters here — a `cdk.out` left behind by an earlier run. This is V13-1's shape exactly (a control never exercised in the environment that decides), and I wrote both. | `make check` now runs `synth` before `test`, verified by deleting `infra/cdk.out` and running the gate clean. Three holes closed rather than one: `tests/unit/test_makefile_env.py` asserts the prerequisite order, so it cannot quietly revert; `tests/unit/synthesised.py` is the single loader for template-reading tests and **fails on a stale `cdk.out` as well as a missing one**, by comparing mtimes against `infra/` — the silent case, which no ordering fix removes for a direct `pytest` run; and `test_every_stack_in_the_tree_is_checked` derives the stack list from the tree, so the flag-gated guardrails stack could not slip past the cost rule by not being in `cdk.out`. |
| **V13-8** | **High** | **Two alarms watched a dimension combination nothing published.** A dimension set is part of a metric's *identity* in CloudWatch, and EMF rolls up across none of them: `{stage, plane}` and `{stage, plane, systemId}` are two different metrics that share a name. `emf_document` published exactly one set — whatever the call site passed — while every alarm in `observability.py` was written against `{stage, plane}`. So `hard_delete`, which dimensions `phase3.stuck_participants` by participant *because* "which system is stuck" is the operator's first question, published only the wide set: the alarm for a halted erasure could never receive a data point. `policy.deny` had the same break on the dashboard. And because M10 had already — correctly — set `TreatMissingData: NOT_BREACHING` on every alarm, the failure did not even present as `INSUFFICIENT_DATA`; it rendered **green**. Three separate checks asserted that alarm and emitter agreed, and all three compared metric *names*. | `Dimensions.dimension_sets()` publishes the base set `{plane, stage}` unconditionally and adds the wide set alongside it, so the alarms keep one shape whatever the call site passes and the per-participant breakdown survives. Two new synth checks derive the expected set from the emitter rather than from a literal, and a third closes `_plane_of`'s adjacent claim — it said it derived the plane "so the two cannot drift silently" and nothing was checking, which is the same defect one field over. All three were confirmed to go red against the old behaviour before being kept. |

| **V13-9** | **Medium** | **A Build-line item that nothing built, and a doc claiming it in the present tense.** M10's Build line lists *"AgentCore Evaluations wired against the dev stack as the drift monitor"*. Nothing in `infra/`, `evals/` or `.github/` creates it. ARCHITECTURE §11.2 and ADR-020 both describe it as *running* — the exact "untested claim" shape this file exists to catch, and the one place M10 produced it rather than removed it. It survived because **neither M10 gate mentions it**: the hermetic gate is about metrics, alarms, cost floors and the workflow; the deployed gate is about the per-PR run, cost and the dashboard. A Build item with no gate is a to-do written in prose. | §11.2 now marks it not-built and says what is actually missing — the *continuous* half, since `discovery.recall` is already emitted from the gate of record and is what the dashboard shows today. Left unbuilt rather than half-built: it is a new AWS surface whose API shape must be verified against current documentation and whose wiring can only be exercised against a deployed stack. **Disposition: deferred by decision on 2026-07-30** to a follow-up demo built around continuous evaluation, with the intended shape recorded in §11.2 so the deferral carries the design rather than losing it. The Build line is struck through, not deleted. A deferred item and a forgotten one look identical in a repo six months later, and this file's whole subject is the difference. |

| **V13-10** | **High** | **`make deploy-guardrails` died mid-deploy, and the hermetic gate was what killed it.** Reported from a real run: `ENOENT: no such file or directory, open '...infra\cdk.out\asdp-account-guardrails.metadata.json'`. Every CDK invocation in the Makefile — the free, hermetic `synth` and all four AWS-mutating targets — shared one output directory. `cdk deploy` synthesises, parses `manifest.json` into memory, then re-opens per-stack files (`additionalMetadataFile`, the asset manifest) **lazily**, minutes later, while CloudFormation works. A concurrent `make check` rewrites `infra/cdk.out` wholesale. The guardrails stack is the sharpest case and the reason the error named it specifically: a plain synth carries no `accountGuardrails` context, so it recreates the seven dev stacks' files and *not* that one — removing precisely the file the in-flight deploy still needed, and nothing else. **I triggered it**: I was running `rm -rf infra/cdk.out && make check` on a 2–5 minute cycle while the human ran the deploy. | The four `deploy`/`destroy` targets synthesise into `CDK_DEPLOY_OUT` (`infra/cdk.out.deploy`); `synth` keeps `infra/cdk.out`, which `tests/unit/synthesised.py` reads. Four checks in `test_makefile_env.py`: no `$(CDK) deploy\|destroy` without `-o '$(CDK_DEPLOY_OUT)'`, the variable must not resolve to the gate's directory, `synth` must keep the default, and `clean` must remove both. Confirmed red against the unfixed Makefile, and confirmed by synthesising the guardrails stack into the deploy directory with the gate's `cdk.out` byte-identical afterwards. **The first fix was incomplete and broke `make check` for a day:** `tests/unit/test_doc_links.py` excluded `cdk.out` by exact name, so it walked `cdk.out.deploy` and graded vendored markdown inside a staged asset. It went unnoticed because the gate run that "proved" the fix had deleted both directories first — the same *delete the artefact and run the gate* habit, hiding a failure this time instead of exposing one. Generated directories are matched by prefix now. |

| **V13-11** | **High** | **`make deploy-dev` failed with a 400: the observability stack's metric filter set two properties AWS forbids together.** *"Invalid metric transformation: dimensions and default value are mutually exclusive properties."* AWS states it plainly — *"If you assign dimensions to a metric created by a metric filter, you can't assign a default value for that metric"* — and `saga.executor_timeout`'s filter set both. **`cdk synth` cannot catch this**: it validates template shape against the resource schema, never the service's own semantics, so the hermetic gate was green and a unit test asserted `DefaultValue == 0` as though it were a requirement. It took a real deploy, and it failed *after* the change set was accepted. | `default_value` dropped, `dimensions` kept — and which one survives is the whole decision. The alarm is built with `dimensions_map={stage, plane}` like every other alarm in the stack, so publishing this metric bare would leave it watching a combination nothing writes: V13-8 re-entered through a service constraint. The zero baseline bought less than it appeared to, since the alarm already treats missing data as `NOT_BREACHING` and a lone timeout fires at neither setting; the dashboard shows gaps rather than a flat zero line, and that is the entire cost. The rule is now mechanical — a check sweeps **every** synthesised template for the forbidden pair, confirmed red against the unfixed stack, so the next metric filter is covered on creation rather than on the deploy it breaks. |

| **V13-12** | **High** | **The second deploy failed on the same resource: `"The specified filter pattern does not support dimensions."`** V13-11's fix kept `dimensions` and dropped `default_value`, on the reasoning that an alarm watching a dimension set nobody publishes can never fire (V13-8). The reasoning was sound and the choice was still wrong, because a third constraint had not been checked: CloudWatch Logs supports dimensions only on a filter pattern that **extracts named fields** — space-delimited `[a, b, c]` or JSON. This filter matches the Lambda runtime's own `Task timed out after N seconds`, which is unstructured text, and `stage`/`plane` are constants of the deployment rather than fields of the event. **No pattern could ever have produced them.** Two deploys, two different 400s, both invisible to `cdk synth` — which validates the template against the resource schema and knows nothing of the service's semantics. | Dimensions dropped and `default_value` restored, which the removal makes legal again — and the zero baseline is what lets the alarm distinguish "no timeouts" from "nothing reported". The alarm and the dashboard now derive their dimension set from one helper, so publisher and watcher cannot disagree; `UNDIMENSIONED` records the exception with the reason, and a test requires anything in it to be stack-published, since application code has no such constraint. Two hermetic guards now encode what the service taught: no metric filter may pair dimensions with a default value, and none may pair dimensions with a pattern that names no fields. Both confirmed red against the unfixed stack. **The cost is disclosed rather than absorbed:** this one metric no longer separates by stage, so a `pr-1234` stack's timeout reaches the dev alarm. The alternative — alarming on `AWS/Lambda` `Duration`, whose `FunctionName` dimension is stage-scoped — costs the single-namespace property the registry is built on, and is recorded in the stack as the trade to make if that collision ever matters. |

| **V13-13** | **Medium** | **A deployed walkthrough polled a halted saga for fifteen minutes, then blamed the clock.** `make walkthrough SUBJECT=sub_mar_7f3a91c4` reached `already_tombstoned` at **four seconds** — the subject had been consumed by an earlier deployed gate — and then printed fourteen identical progress lines before failing with *"did not reach 'approval' within 900s"*. That message sends an operator looking for slowness in discovery when the saga had halted, correctly, almost immediately. **The control for this already existed and had been written for exactly this failure**: `wait_for`'s docstring promises that a saga halting elsewhere "ends the wait immediately… The alternative is polling a finished saga until the timeout and then reporting 'did not reach the gate', which is true, useless, and fifteen minutes late." `already_tombstoned` was simply missing from `TERMINAL_STATUSES` — and the comment directly above that set records `no_data` having done the same thing at V11-4. | Added, with an explanation that names the actual remedy (the seeded subjects are one-shot; pick another or re-seed). **The interesting part is why the existing test did not catch it:** `test_the_terminal_set_and_the_explanations_agree` compares the CLI's two hand-maintained lists *to each other*, and `already_tombstoned` was absent from **both** — so they agreed, and the test passed while both were wrong. Two lists agreeing is not evidence either is right, and the source of truth was always `saga/state.py`. A new check reads the `STATUS_*` constants from there and requires every one to be classified as terminal or explicitly non-terminal, so adding a status to the saga now fails the build until someone decides which it is. A behavioural test asserts the wait actually ends — **with `POLL_TIMEOUT_SECONDS` monkeypatched to 1**, because written against the real value a regression does not fail, it stalls `make check` for fifteen minutes and reads as a hang. Both confirmed red in about a second against the unfixed module. |

| **V13-14** | **Medium** | **Two error messages advised a remedy that cannot work, and I wrote the second one by copying the first.** V13-13's new explanation told the operator to "re-run `make seed` to restore the fabricated set"; `seeded_subject()` had said "run `make seed` for a fresh set" since M8. **Neither can succeed.** The seven `subjectRef` values are hardcoded in `seeds/meridian.json`, and the tombstone registry is keyed by their hash and is append-only *by design* — §5.3's anti-resurrection control, whose entire point is that an entry outlives the data permanently. So `make seed` rewrites the same seven refs, every one still tombstoned, and intake refuses exactly as before. The command succeeds, nothing changes, and the operator concludes the platform is broken. Advice that routes someone to a command which appears to work and changes nothing is worse than no advice. | Both messages corrected to state the constraint and the only real reset — a new stack. Deliberately *not* offered: deleting rows from the tombstone table, which would work and would teach an operator to disable the control this platform exists to demonstrate. `--subject` is also now validated **before** a saga is created, so naming an erased subject costs one DynamoDB read instead of a saga, and the refusal names which subjects remain usable. Scope was narrowed during review: the first version also refused any subject absent from the local ground-truth map, which broke `test_an_explicit_subject_is_honoured` — `--subject` is a debugging escape hatch, and refusing on absence turns it into a whitelist keyed on a gitignored build artefact. The check now overrules the operator only where the fixtures *know* the run cannot work: erased, or under a legal hold. |
| **V13-15** | **High** | **Every WORM plan failed manifest validation, and a unit test asserted the exact value that could not be produced.** `reconcile` set `"deleteMethod": "CRYPTO_SHRED" if archetype is Archetype.WORM else "PURGE"` and never set `dekRegistryRef` — the string appears zero times in the file. `ManifestParticipant` refuses that pair, because a shred naming no target is not a deletion plan. So the participant list came back empty, the saga died in `plan` and retried, and `make walkthrough` printed fifteen identical `status=running gate=None` lines before failing at 900s. The operator sees a timeout and goes looking for a slow discovery agent; the saga had been failing validation since the fourth second. **Why nothing caught it is the part worth keeping.** `compliance-archive` is the only WORM participant and one fixture subject reaches it, so the blast radius looked small — but the unit fixture handed *every* system the same generic `{"kind": "row"}` artifact, and `test_the_worm_shred_sorts_last_and_derived_stores_first` asserted `deleteMethod == "CRYPTO_SHRED"` against it and passed. The fixture was not standing in for the participant, it was agreeing with the caller. Underneath sat a second defect that made the first one invisible: `"wrapped-dek"` was defined in `participants/compliance_archive` alone, while the planner had to match on it — a vocabulary two layers share and one layer owns, which is a coupling that shows up in no import graph. | `DEK_ARTIFACT_KIND` moved to `contract/archetypes.py`, so participant and planner read one definition; the participant's local constant now aliases it rather than repeating the string. `_deletion_method` fills the ref from the locator the participant returned in `discover`, **not** by re-deriving `table#subject` — a second address for the key that can drift from the key it addresses is precisely the side mapping ADR-021 warns against. **WORM with no wrapped DEK now claims no deletion method at all:** `PURGE` would be a lie, since COMPLIANCE mode refuses deletion to everyone including root, and `CRYPTO_SHRED` would name a target that does not exist — so the entry says nothing and invariant 7's residual machinery discloses it. The fixture was taught what the service returns (locked ciphertext *and* wrapped DEK), which makes the original assertion true rather than lucky, and three tests pin the branches: a shred carries the reported DEK, a keyless WORM system claims no method, a non-WORM system purges and names no key. All three are red against the unfixed module by construction — `dekRegistryRef` is absent from it entirely, and `deleteMethod` is set unconditionally. **Still owed: the deployed re-run.** This was found by a deployed walkthrough and only a deployed walkthrough closes it; the hermetic gate can prove the planner now emits a valid shred, not that the shred works. |

**V13-3 is the third instance of one class**, after V11-6 (digests and ARNs) and its own follow-up: *a structural value corrupted by a rule written for free text*. The fix each time has had to be the class rather than the case, and the tell is the same — a value that must survive byte-identical, passing through a redactor whose rules were designed for prose.

**V13-4 is a different and more interesting shape: a rule with a mechanism-shaped sentence and no mechanism.** ADR-021 said "check before adding a service", which is an instruction to a human at review time — the weakest kind of control, and the one that had already failed once (OpenSearch's OCU floor is *why* ADR-021 exists). The new check deliberately lists **twenty-odd services this repo has never used**, because a denylist of what is already present passes forever and guards nothing. It also makes the Aurora acceptance *conditional*: `min_capacity = 0` is asserted against the template, since "we chose Serverless v2" is not the property that makes it free, and a 0.5-ACU floor would be about $43/month while passing any check that only looked at the engine.

**On finding your own claim to be slightly false.** The cost sentence was not a lie and the number was not material — $0.40 against a Bedrock bill is noise. The reason it is recorded anyway is that an absolute claim with one silent exception teaches a reader to trust the *next* absolute claim by the same author, and this repo's only real asset is that its claims can be checked.

**V13-7 is the one to be least comfortable about.** Not because it is the hardest — it is a one-word reordering — but because the pass that found V13-1 ("a control that has never run in CI is a claim") then shipped another one, three commits later, in the file whose entire purpose is to make a rule mechanical. The mechanism that hid it is worth naming: a build artefact in the working tree makes a test that depends on it pass forever locally, and the developer's own machine is the one environment where the dependency is always satisfied. The general lesson is narrower than "run it in CI" — it is **delete the artefact and run the gate**, which took eleven seconds and is now what proves the fix.

**V13-10 is V13-7's directory again, and this time the gate was the aggressor.** Same `infra/cdk.out`, opposite direction: V13-7 was a test reading an artefact a *previous* run left behind, V13-10 was a deploy losing an artefact to a *concurrent* run. The two share a root that neither fix on its own addresses — one mutable directory with several owners, some free and re-runnable, one of them mid-flight against real infrastructure. Separating them is the actual repair; "delete the artefact and run the gate" is exactly the habit that caused the second one, so the lesson from V13-7 was, in a small way, the mechanism of V13-10. **A shared build artefact is a concurrency surface, and a gate that is safe to run at any time has to be safe to run at any time.**

**V13-5 and V13-6 are the same question asked twice: *what number does this actually produce?*** Both controls were specified from the requirement rather than from the system. A one-month statutory deadline says a 23-day alarm; a silent saga says somebody should be told. Neither statement survives contact with the arithmetic — a 30-day grace window inside a one-month deadline means the healthy case breaches, and a metric emitted by the thing that has stopped running cannot report that it stopped. **Four ways an alarm can be useless, and M10 has now met all four:** watching a metric nobody publishes (`INSUFFICIENT_DATA` forever), watching a metric whose name the scrubber corrupted (V13-3), watching a threshold the healthy case exceeds (V13-6), and watching a *dimension set* nobody publishes (V13-8). The third is the most dangerous to an operator, because it *does* fire — and then gets muted, which is a missing alarm that also looks like diligence in the commit log.

**V13-8 is the most uncomfortable of the eight, and the reason is structural.** M10 was built around one rule, stated in three module docstrings and asserted by three separate tests: never alarm on a metric nothing publishes. The rule held. What failed is that all three checks compared metric **names**, and a CloudWatch metric is not identified by its name — it is identified by its name *and its dimension set*. The milestone's own defence, `TreatMissingData: NOT_BREACHING`, then converted the symptom from `INSUFFICIENT_DATA` into a green tile, which is to say the correct fix for one failure mode hid another. The lesson is not "check dimensions too". It is that **an agreement test is only as strong as its notion of identity**, and this one had been written from the vocabulary of the doc (§10.1 lists names) rather than from the vocabulary of the service.

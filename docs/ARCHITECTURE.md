...
So: **no servers, no clusters, no VPC, and no always-on compute anywhere in the request path.** The reasoning plane is AgentCore Runtime (microVM per session, scale-to-zero). The control plane is Lambda plus a DynamoDB checkpointer. The timers are EventBridge Scheduler. The participants are Lambda functions in front of managed AWS services reached over their public, SigV4-authenticated APIs — including Aurora, via the RDS Data API, specifically so that nothing needs a VPC attachment.
...
| 016 | DynamoDB checkpointer + EventBridge Scheduler own durability | Fully serverless, no VPC, zero idle cost | Aurora Serverless v2; AgentCore Memory as checkpoint store |
